import scrapy, os, datetime, pureGrap.myLogger, xlsxwriter, shutil, re
...
class WhatshappeningsgSpider(scrapy.Spider):
    ...
    def __init__(self, gday=None, gmonth=None, mdir=None, *args, **kwargs):
        ...
        self.myheaders = {'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
                          ...
                          'Connection': 'keep-alive', 'Host': 'www.whatshappening.sg',
                          'Referer': 'http://www.whatshappening.sg/events/index.php?d=2016-09-22&m=1',
                          'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.116 Safari/537.36',
                          'Upgrade-Insecure-Requests': '1'}
        self.myCookie = {"helios_9f9cd63c14a22c52f62e358535f47b8f": "hgla5v6v0t3jo2kdk7bge6ci57", "__utmt": "1",
                         "__utma": "217362000.774928566.1474351692.1474421244.1474423088.3",
                         "__utmb": "217362000.2.10.1474423088", "__utmc": "217362000",
                         "__utmz": "217362000.1474351695.1.1.utmcsr=(direct)|utmccn=(direct)|utmcmd=(none)",
                         "__unam": "2f1607e-15746358f6a-69697270-13"}
        try:
    ...
    def start_requests(self):
        ...
        yield scrapy.Request(url='{}?d={}&m=1'.format(self.siteDomain, timeStr), headers=self.myheaders,
                             cookies=self.myCookie, dont_filter=True, callback=self.parse)
    ...
    def parse(self, response):
        ...
        for onee in trCollections:
            ...
            yield scrapy.Request(url=eventUrl, headers=self.myheaders, cookies=self.myCookie, dont_filter=True,
    ...
    def parseOneEvent(self, response):
        try:
            ...
            if len(tlongitude) > 0:
                ...
                longitude = 0
            address = '{} {}'.format(pageData.select("[itemprop=streetAddress]")[0].contents[0],
                                     pageData.select("[itemprop=addressCountry]")[0].contents[0])
            timage = pageData.select("[itemprop=description]")[0].select("img")
            if len(timage) > 0:
                image = timage[0]["src"]
            else:
                image = ""
            self.eventResults.append(
                {"title": title, "image": image, "description": description, "startDate": startDate, "endDate": endDate,
                 "latitude": latitude, "longitude": longitude, "eventUrl": response.url, "requires": "",
                 "address": address})
        except:
