...
class IMDBSpider(scrapy.Spider):
    ...
    def __init__(self, myear=None, mdir=None, *args, **kwargs):
        super(IMDBSpider, self).__init__(*args, **kwargs)
        self.syear = myear
        if mdir is None:
            self.topDir = "D:/troopar/imdb/movie"
        else:
            self.topDir = "{}/imdb/movie".format(mdir)
        ...
        self.myheaders = {"Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8",
                          ...
                          "Referer": "http://www.imdb.com/search/title?release_date=2016&view=advanced",
                          "Host": "www.imdb.com", "Cache-Control": "max-age=0", "Connection": "keep-alive"}
        self.mycookie = {}
    ...
    def start_requests(self):
        for i in range(200, 250):
            pageUrl = "http://www.imdb.com/search/title?release_date={}&view=advanced&page={}&ref_=adv_prv".format(
                self.syear, str(i))
    ...
    def parse(self, response):
        ...
        for item in items:
            itemHeader = item.find("div", attrs={"class": "lister-item-content"}).find("h3", attrs={
            ...
            yield scrapy.Request(url=movieUrl, headers=self.myheaders, meta={"indexno": indexno}, cookies=self.mycookie,
    ...
    def parseSingle(self, response):
        ...
        if boxofficeurl is not None:
            ...
            yield scrapy.Request(url=boxofficeurl, headers=self.myheaders,
                                 meta={"pageNo": str(pageNo), "pageId": pageId["content"], "contens": contens},
                                 cookies=self.mycookie, callback=self.getboxoffice)
        else:
