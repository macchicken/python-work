import scrapy,os
from pureGrap.items import PuregrapItem


class BeautylegSpider(scrapy.Spider):
	name = "beautyleg"

	def __init__(self, aurl=None, mname=None, mcookie=None, *args, **kwargs):
	...
	def start_requests(self):
		yield scrapy.Request(url="http://www.beautyleg.com{}".format(self.albumUrl), headers=self.myheaders, cookies={"PHPSESSID":self.myCookie},callback=self.parse)

	def parse(self, response):
		for aLink in response.css('a'):
			fileUrl=aLink.xpath('@href').extract()[0]
			fileName=fileUrl.split('/')[-1:][0]
			yield scrapy.Request(url="http://www.beautyleg.com{}".format(fileUrl),headers=self.myheaders,cookies={"PHPSESSID":self.myCookie},meta={'fileName': "{}/{}".format(self.albumDir,fileName)},callback=self.saveFileToDisk)
