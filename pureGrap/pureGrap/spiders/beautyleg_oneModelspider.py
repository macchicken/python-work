...
class BeautylegOneModelSpider(scrapy.Spider):
    ...
    def __init__(self, mname=None, mcookie=None, mdir=None, *args, **kwargs):
    ...
    def start_requests(self):
        with open(self.modelFile, "rb") as tempRead:
            for line in tempRead:
                albumUrl = line.strip("\n")
                yield scrapy.Request(url="http://www.beautyleg.com{}".format(albumUrl), headers=self.myheaders,
                                     meta={'albumUrl': albumUrl}, cookies={"PHPSESSID": self.myCookie},
                                     callback=self.parse)
    ...
    def parse(self, response):
        ...
        for aLink in response.css('a'):
            ...
            yield scrapy.Request(url="http://www.beautyleg.com{}".format(fileUrl), headers=self.myheaders,
                                 ...
                                 meta={'fileName': "{}/{}".format(albumDir, fileName)}, callback=self.saveFileToDisk)
