    ...
    def parse(self, response):
        for aLink in response.css('a'):
            ...
            if "video_no" in videoLink:
                yield scrapy.Request(url="http://www.beautyleg.com/member/{}".format(videoLink), headers=self.myheaders,
                                     cookies={"PHPSESSID": self.myCookie}, callback=self.saveActualLink)

    def saveActualLink(self, response):
        actualLink = response.css("a").xpath('@href').extract()[0]
        if not actualLink.startswith("http:"):
            self.videoLinks.append("http://www.beautyleg.com{}".format(actualLink))
        else:
